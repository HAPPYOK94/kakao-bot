if (msg.indexOf("/날씨 ") == 0) {
  ...
  try {
    ...
    var weatherarea = org.jsoup.Jsoup.connect(
      "https://m.search.naver.com/search.naver?query=" + cmd + "날씨"
    )
      .get()
      .select("div.title_wrap")
      .select("h2")
      .text();
    ...
    var weatherdata = org.jsoup.Jsoup.connect(
      "https://m.search.naver.com/search.naver?query=" + cmd + "날씨"
    )
      .get()
      .select("div.status_wrap");

    var weathertom = org.jsoup.Jsoup.connect(
      "https://m.search.naver.com/search.naver?query=" + cmd + "날씨"
    )
      .get()
      .select("div.inner_box");

    var wtmain = weatherdata.select("div.weather_main").get(0).text(); // 현재 날씨
    var nowtem =
      weatherdata
        .select("div.temperature_text")
        .get(0)
        .text()
        .replace("현재 온도", "")
        .replace("°", "") + " ℃"; // 현재 온도
    var uptem =
      weatherdata.select("dd.up_temperature").text().replace("°", "") + " ℃"; // 최고 온도
    var dntem =
      weatherdata.select("dd.down_temperature").text().replace("°", "") + " ℃"; // 최저 온도
    var fltem =
      weatherdata
        .select("dd.feeling_temperature")
        .text()
        .replace("체감", "")
        .replace("°", "") + " ℃"; // 체감 온도
    ...
    var rpsp = reportlist
      .select("li.type_report report8")
      .select("span.figure_text")
      .text();
    ...
    weatherStr =
      ...
      "\n        강수확률 : " +
      weathertom.select("strong").get(1).text();
    weatherStr =
      weatherStr +
      "\n\n내일 오후 날씨 : " +
      ...
      "\n        강수확률 : " +
      weathertom.select("strong").get(3).text();
